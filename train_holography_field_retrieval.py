...
classifier = models.vgg19(weights=None, num_classes=1)
...
# args.holo_weight = 0
holo_weight_final = 10
step = 1000
inc = holo_weight_final/(args.max_iter/step)
...
for i in tqdm(range(args.max_iter)):
    ...
    if args.unknown_distance:
        loss_c, loss_s, c2s_amp, c2s_ph, style_re, distance_content = network(content_images, style_images, field_retrieval=True, unkonwn_distance=True)
    ...
    s2c_re = model_forward(c2s_amp, c2s_ph, distance_content-distance_style).sqrt()
    s2c_foc = model_forward(c2s_amp, c2s_ph, -distance_style, complex_number=True)
    ...
    loss = loss_c + loss_s + loss_phy_con + loss_identity + loss_tv
    ...
    if (i + 1) % args.vis_interval == 0 or (i + 1) % args.save_model_interval == 0 or (i + 1) == args.max_iter:
        ...
        for j in range(8):
            with torch.no_grad():
                ...
                amp_foc, ph_foc = model_forward(amplitude, phase*args.phase_normalize, -distance_style[j:j+1], return_field=True)
                if args.unknown_distance:
                    prop=model_forward(amplitude, phase*args.phase_normalize, distance_content-distance_style[j:j+1]).sqrt()
                else:
                    prop=model_forward(amplitude, phase*args.phase_normalize, distance_content[j:j+1]-distance_style[j:j+1]).sqrt()
