...
class Net(nn.Module):
    ...
    def forward(self, content, style, alpha=1.0, field_retrieval=False, unkonwn_distance=False):
        ...
        if unkonwn_distance:
            d_content = self.distance_g(calc_mean_std(t))
            return g_t, g_t_phase, d_content
        else:    
    ...
    def field_retrieval(self, content, style_feats, alpha=1.0, unknown_distance=False):
        ...
        if unknown_distance:
            if t.shape[0] == 1:
                return g_t, g_t_phase, self.distance_g(calc_mean_std(t.repeat(2, 1, 1, 1)))[:1, :] # self.distance_g(content_feat.repeat(2, 1, 1, 1))[:1, :]
            else:
                return g_t, g_t_phase, self.distance_g(calc_mean_std(t))
        else:
            return g_t, g_t_phase
